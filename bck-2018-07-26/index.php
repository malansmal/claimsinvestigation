...
<div id="allbelowmenu">
	...
	echo "<h3>Administrators Login:</h3>
		...
		echo "<h3>Claims Technician Login:</h3>

		<form action=\"admin/ccloggedin.php\" method=\"post\">
	...
	$qrycc = "select * from `claimsclerks` order by `name`";

	$qryccresults = mysql_query($qrycc, $db);

	

	while ($ccrow = mysql_fetch_array($qryccresults))
...
";
	...
	echo "</select></td>
			...
			<tr>
				...
				<td><input type=\"password\" name=\"ccpassword\"></td>
		...
		echo "<h3>Assessors Login:</h3>

		<form action=\"admin/asloggedin.php\" method=\"post\">

		<table id=\"logintable\">

			<tr>

				<td>Select Assessor:</td>

				<td><select name=\"assessor\">";

				

		$qryass = "select * from assessors order by `name`";

		$qryassresults = mysql_query($qryass, $db);

				

		while ($assrow = mysql_fetch_array($qryassresults))

		{
			...
			echo "<option value=\"$assid\">$assname</option>";
		...
		echo "		</select>
			...
			<tr>
				...
				<td><input type=\"password\" name=\"aspassword\"></td>
	...
	echo "	<h3>Panel Beaters Login:</h3>

			<form action=\"admin/pbloggedin.php\" method=\"POST\" name=\"pbloginform\">

				<table>

					<tr>

						<td>Select Panel Beater:</td>

						<td><select name=\"selectpb\">

						    <option value=\"0\">Select Panel Beater</option>";

	

	$qrygetpb = "select * from panelbeaters order by `name`";

	$qrygetpbresults = mysql_query($qrygetpb, $db);

	

	while ($pbrow = mysql_fetch_array($qrygetpbresults))
	...
	echo "</select></td>
					...
					<tr>
						...
						<td><input type=\"text\" name=\"claimnumber\" /></td>
...
?>
    ...
    <div id="footer">
      ...
      &copy;  Auto Claims Investigation all rights reserved | <a href="http://www.o5webdesign.com" target="_blank">Website Design</a> by o5webdesign
