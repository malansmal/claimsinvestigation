<?php
	...
	function TowingOperators($request_data){
		...
		echo "
			...
			<p style='margin-bottom:20px;visibility:hidden;' id='top-actions'><a href='#' id='btn-send-to-all' class='btn btn-primary'><i class='glyphicon glyphicon-envelope'></i> Send email to selected towing operators</a></p>
		";
...

		{
				  ...
				  <table class=\"table table-striped\">
						  ...
						  <tr>
							<td><input type='checkbox' id='checkAll' title=\"Select all\" /></td>
							  ...
							  <td colspan=\"4\" align=\"center\"><strong>Actions</strong></td>
...

			{
				...
				echo "<tr>
						  ...
						  <td><input type=\"checkbox\" class=\"row-checkbox\" name=\"towingoperator-checked[]\" value=\"$id\" /></td>
				...
				echo "
						  ...
						  <td align=\"center\"><a href=\"loggedinaction.php?action=edittowingoperator&amp;towingoperatorid=$id\"><img src=\"../images/admin/edit.gif\" alt=\"Edit this Towing Operator\" border=\"0\" title=\"Edit this Towing Operator\"></a></td>

						  <td align=\"center\"><a href=\"loggedinaction.php?action=confirmdeletetowingoperator&amp;towingoperatorid=$id\"><img src=\"../images/admin/delete.gif\" alt=\"Delete this Towing Operator\" border=\"0\" title=\"Delete this Towing Operator\"></a></td>

						  <td align=\"center\"><a href=\"loggedinaction.php?action=send-profile-link-to-towingoperator&amp;towingoperatorid=$id\" onClick=\"return confirm('Are you sure, you want to send profile updation link to this Towing Operator?');\"><img src=\"../images/email-send.png\" style=\"height:16px;\" alt=\"Send Profile Update link to this Towing Operator\" border=\"0\" title=\"Send Profile Update link to this Towing Operator\"></a></td>
			...
			echo "<tr>
					  ...
					  <td colspan=\"4\" align=\"center\"><a href=\"loggedinaction.php?action=newtowingoperator\"><img src=\"../images/admin/add.gif\" alt=\"Add new Towing Operator\" border=\"0\" title=\"Add new Towing Operator\"></a></td>
			...
			<div class=\"modal-dialog\">
			  ...
			  <div class=\"modal-content\">
				<div class=\"modal-header\">
				  <h4 class=\"modal-title\">Sending Emails to Towing Operators</h4>
				</div>
				<div class=\"modal-body\">
				  <p>Please wait while the system is sending emails to the Towing Operators. This may take upto a couple of minutes.</p>
				</div>
			  </div>
		  ...
		  <script type='text/javascript'>
				...
				$('#btn-send-to-all').click(function(event){
					...
					var agree = confirm('Are you sure, you want links to all the selected towing operators?');
					...
					var url = 'loggedinaction.php?action=send-profile-link-to-towingoperator&towingoperatorid=' + pbIds;
...
<br><input type=\"submit\" value=\"Save\"> <input type=\"reset\" value=\"Clear\"> <input type=\"hidden\" name=\"towingoperatorsid\" value=\"$towingoperatorid\">
...

	{
		...
		$cars = (int) $_REQUEST["cars"];

		$hcv = (int) $_REQUEST["hcv"];

		$roll_back = (int) $_REQUEST["roll_back"];
	...
	function SendProfileLinkToTowingOperator($twid) {
		
		require('connection.php');

		$towingoperatorids = explode(',', $twid);

		require_once "../vendor/autoload.php";

		$mail = new PHPMailer;

		foreach ($towingoperatorids as $twid) {
		
			$guid = generate_guid();

			$sql = " UPDATE towingoperators SET profile_access_token = '" . $guid . "' WHERE id='" . $twid . "' AND ( profile_access_token='' OR profile_access_token IS NULL ) ";

			mysql_query($sql);

			$guidSQL = " SELECT `profile_access_token` FROM towingoperators WHERE id='".$twid."' ";
			$guidResult = mysql_query($guidSQL);
			$guidInfo = mysql_fetch_object($guidResult);
			$guid = $guidInfo->profile_access_token;


			$profileLink = 'http://' . $_SERVER['HTTP_HOST'] . '/update-towingoperator-profile.php?towingoperatorid=' . $twid . '&token=' . $guid;

			$twQry = " SELECT name, email FROM towingoperators WHERE id='" . $twid . "' ";

			$res = mysql_query($twQry);

			$twInfo = mysql_fetch_object($res);

			$mail->setFrom('info@panelshop.co.za', 'Panelshop.co.za');
			$mail->addAddress($twInfo->email, $twInfo->name);     // Add a recipient
			$mail->addReplyTo('info@panelshop.co.za', 'Panelshop.co.za');
			$mail->addBCC('info@panelshop.co.za');

			$mail->isHTML(true);                                  // Set email format to HTML

			$mail->Subject = 'Please update your Towing Operator Profile for Insurance Purposes';

			$body = file_get_contents('../templates/towingoperator-update-profile-notification.html');

			$body = str_replace(['{{TOWINGOPERATOR_NAME}}', '{{PROFILE_UPDATE_LINK}}'], [$twInfo->name, $profileLink], $body);
		...
		$_SESSION['success_message'] = 'Sent profile update link to Towing Operators.';

		?>
			<script type="text/javascript">
				location.href = 'loggedinaction.php?action=towingoperators&from=1';
			</script>
